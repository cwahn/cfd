...
class PDE_pruned_UNet(nn.Module):
	...
	def __init__(self):
		...
		# Bridge
		self.bridge = conv_block_3d(self.num_filters*2, self.num_filters*4, activation)
	...
	def forward(self, mesh_voxel):
		
		x = torch.cat([mesh_voxel],dim=1)
		
		# Down sampling
		down_1 = self.down_1(x) # N, 8, D
		pool_1 = self.pool_1(down_1) # N, 8, D/2
		
		down_2 = self.down_2(pool_1) # N, 16, D/2
		pool_2 = self.pool_2(down_2) # N, 16, D/4
		...
		f_velocity, f_pressure, f_kappa = out[:,0:3], out[:,3:4], out[:,4:5]

		return f_velocity, f_pressure, f_kappa

class aaaaaaaaaaaaaaaaaaaPDE_pruned_UNet(nn.Module):
